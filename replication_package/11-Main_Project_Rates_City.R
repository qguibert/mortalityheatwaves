...
#####################################################################################################
#' @Information :
#' Forecast mortality rates with temperature effects for one city
#####################################################################################################

rm(list=ls(all=TRUE))

#----- PARAMETERS TO CHANGE BY THE REPLICATOR ----------------------------------
# If necessary, please define the .libPaths
# .libPaths("/home/quentinguibert/R/x86_64-pc-linux-gnu-library/4.2")
...
# list city
tab_city <- read_excel("data/city.xlsx")
...
for(selected_city in c("Brest", "Marseille", "Paris", "Perpignan", "Strasbourg"))
{
  # Define folder
  loc_folder <- paste0("adj_proj_rates_", selected_city)
  ...
  clim_model <- lapply(1:nrow(info_model), function(i)
  {
    ...
    treat_rcp(info_model$Id[i], info_model$RCP[i],info_model,
              ...
              new_calcul= FALSE, city = TRUE, fold_data, fold_data_raw)
  })
  clim_model <- do.call("rbind", clim_model)

  # # Import city
  clim_model <- clim_model %>%
    left_join(tab_city, by = c("LAT", "LONG")) %>%
    ungroup() %>%
    dplyr::select(-c(LAT, LONG))

  # Select a city
  clim_model <- clim_model %>%
    dplyr::filter(CITY == selected_city) %>%
    ungroup() %>%
    dplyr::select(-c(CITY))
