...
#' @param arima_spec The ARIMA time series specifications for \mjeqn{(K_t^M, \kappa_t^M, \kappa_t^F)}{ASCII representation}.
...
#' \mjeqn{(\epsilon_t^M, \delta_t^M, \delta_t^F)}{ASCII representation}
#' of the multivariate time series \mjeqn{(K_t^M, \kappa_t^M, \kappa_t^F)}{ASCII representation}.
...
project_parameters <- function(fit_M, fit_F, n_ahead, n_sim, arima_spec, est_method){
  ...
  ts_list <- list(K.t_M = fit_M$K.t, k.t_M = fit_M$k.t,
                  k.t_F = fit_F$k.t)
  m_list  <- list(K.t_M = arima_spec$K.t_M, k.t_M = arima_spec$k.t_M,
                   k.t_F = arima_spec$k.t_F)
  ...
  kappa_sim[["K.t_F"]] <- output[[1]] # add artificial trend for female similar to male to facilate the link with other functions of the package
...
port_estimation <- function(fit_M, fit_F, ts_list, m_list){
  ...
  nll <- nlminb(start = bin, objective = obj_f_port, gradient = NULL, hessian = NULL, X = X, Y = Y, N = nPar.vec,
                ...
                upper = c(ifelse(bin1 == 1, 1-10^-4, Inf), rep(Inf,length(bin2))))
