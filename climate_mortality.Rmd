...
```
This R Markdown Notebook provides an example of numerical applications, as presented
...
The replication of the article’s results can be 
performed from the `replication_package` directory.
...
instal.import.package <- function(package){
  ...
  if(!(paste0("package:", package) %in% search())) library(package, 
                                                           character.only=TRUE,
                                                           quietly = T)
}
...
-   `DateDec`: the date,
-   `age_bk`: age buckets,
-   `Nombre_de_deces`: number of deaths,
-   `years`: years,
-   `TAVG`: average temperature of the day
-   `time`: day number as a integer number
-   `dow`: day of the week as a integer number
-   `month`: month number
-   `Ind_canicule` : not used here. 

Our DLNM model for `Nombre_de_deces` is fitted based on the `TAVG` variable, as
...
the estimated daily death counts for each day $d$ of year $t$.
...
xs_mort <- lapply(names(daily_data), function(x){
  res <- excess_mort_dlnm(daily_data[[x]],
                          fit[[x]],
                          q_range = c(q025, q975),
                          nsim = nsim, 
                          parallel = parallel, 
