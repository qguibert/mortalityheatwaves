...
$$
		\sum\limits_{t \in \mathcal{T}_y} K_{t} = 0 \text{ and } \sum\limits_{x \in \mathcal{X}} 
		B_{x}^2 = 1,
$$
$$
...
$$
where $T_{x,t}^ {(g)}  = (1 - \theta^{(g)}_{x,t})^{-1}$, 
...
$$
\widehat{m}_{x,t}^{(g)} = 
 \widetilde{m}_{x,t}^{(g)} \left[ 1 + \overbrace{\sum_{d \in D}{\omega_{x,t,d}^{(g)} \left( \exp{\left(s(\vartheta_{d,t} ; L, \widehat{\thetavec}_k ) \right)} - 1 \right) \mathbf{1}_{\left\lbrace d \in D\right\rbrace }}}^{\theta_{x,t}^{(g)} (D)/ (1 - \theta_{x,t}^{(g)} (D))} \right],
$$
...
$$
q_{x,t}^{(g)} = 1 - \exp{\left(-\widehat{m}_{x,t}^{(g)}\right)}, \quad
